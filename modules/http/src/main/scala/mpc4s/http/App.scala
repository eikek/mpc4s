...
import mpc4s.http.routes.{CustomContent, Endpoint, Thumbnail}
...
final class App[F[_]](coreConfig: ServerConfig[F], cache: PathCache[F], thumb: Thumbnail[F], mpds: Mpds[F])
  ...
  def endpoints: Route[F] =
    withDefaultHeaders(choice(
      "api"/"v1"/cut(Endpoint(coreConfig, cache, thumb, mpds, config.baseUri.path/"api"/"v1")),
...
object App {
  ...
  def create[F[_]: Effect](cfg: AppConfig, protocolConfig: ProtocolConfig, playerRoute: Option[Route[F]])
    ...
    } yield App(cfg, cache, thumb, mpds)
