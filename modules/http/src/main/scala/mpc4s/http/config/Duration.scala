...
object Duration {
  ...
  def millis(n: Long) = Duration(n)

  def seconds(s: Double) = s.seconds

  def between(ta: Temporal, tb: Temporal): Duration =
    fromJava(JDur.between(ta,tb))

  // def between(ta: Timestamp, tb: Timestamp): Duration =
  //   Duration(tb.millis - ta.millis)

  def betweenNanos(na: Long, nb: Long): Duration =
    Duration((nb - na) / 1000 / 1000)
